...
public class LogInE2ETest extends IntegrationTest {
  ...
  void logUserIn() throws Exception {
    ...
    var tokenResponse =
        objectMapper.readValue(response.getResponse().getContentAsString(), TokenResponse.class);
  ...
  void logUserInWithInvalidUsername_SendBadRequest() throws Exception {
    ...
    var errorResponse =
        objectMapper.readValue(response.getResponse().getContentAsString(), ErrorResponse.class);
