...
public class UpdateCampaignCommandTest {
    ...
    @Test
    void updateCampaign() {
        var command = new UpdateCampaignCommand(campaignId, "SOCIAL_MEDIA", 10_000d, "2025-03-01T08:00:00", "2025-04-01T14:00:00");

        var handler = new UpdateCampaignCommandHandler(campaignRepository);

        handler.handle(command);

        var result = campaignRepository.findById(campaignId).get();
    ...
    void updateCampaignPartially() {
        var command = new UpdateCampaignCommand(campaignId, "SOCIAL_MEDIA", null, null, "2025-10-01T14:00:00");
